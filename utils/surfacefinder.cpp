...
class GluingGraph {
   ...
   public:
    ...
    std::vector<regina::Triangulation<2>> &findEmbeddedSurfaces(
        regina::Triangle<n> *triangle) {
...
void identifyClosedSurface(regina::Triangulation<2> tri) {
    if (!tri.isClosed() || !tri.isConnected()) {
        throw regina::InvalidArgument(
            "tri must be a closed, connected surface!");
    }
...
int main(int argc, char *argv[]) {
    ...
    std::set<ClosedConnectedSurface> closedSurfaces;
    int surfaceCount = 0;
    int closedCount = 0;
    ...
    if (closedSurfaces.size() == 0) {
        ...
        for (auto &closedSurface : closedSurfaces) {
            std::cout << "- " << closedSurface << "\n";
        }
    }
