#!/usr/bin/env python3
"""Builds utils/surfer/data/census.sqlite: a local SQLite census of the 3
Regina census databases relevant to utils/surfer's boundary-complement
lookups (regina::Census::lookup() on a cusped Triangulation<3> can only
ever match one of these), plus SnapPy-identified isoSigs harvested from
custom_analysis/token_identification_cache.json that Regina's own census
misses.

Queried at runtime by census::localCensusLookup() (identifycomplement.cpp)
via one read-only SQLite connection per thread -- no hand-rolled mutex
needed, unlike the real regina::Census::lookup(), whose Tokyo Cabinet
backend is not safe under concurrent opens (see identifycomplement.h's
identify::censusLookupMutex comment). A local miss falls through to that
real, mutex-guarded lookup unchanged, so this file only needs to be *a*
correct subset, never the sole source of truth.
...
identify::identify()'s bare fallback identifier, and therefore
...
census::localCensusLookup() then only ever needs to query by isoSig,
matching every other lookup in this program, with no signature-generation
bookkeeping at runtime. SnapPy-sourced rows need no such conversion: their
tokens already come from identify::identify()'s isoSig fallback.
...
with a new search's results -- this script always regenerates the census
from scratch, so it stays a pure function of those two checked-in inputs.

WARNING: verifyslicegenus (see ../verifyslicegenus.cpp) also extends this
same file live, in C++, as it runs (census::insertCensusEntry(), tagged
source='verifyslicegenus'/'retriangulate') -- since this script always
deletes and rebuilds the output from scratch, re-running it after
verifyslicegenus has been run discards those live-inserted rows. If you
want to keep them, back up data/census.sqlite (or extract its
source != 'regina'/'snappy' rows) before re-running this script.
"""
import argparse
import json
import lzma
...
DEFAULT_OUTPUT = SURFER_DIR / "data" / "census.sqlite"
...
# the latter is meaningful as a census key.
