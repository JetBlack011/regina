...
`surfer` performs a DFS over a "gluing graph" of triangles to find all
...
| `surfer.h` | `SurfaceFinder<dim>`: builds the graph of triangles/gluings that the DFS runs over (using `GluingNode<dim>` from `gluing.h`), and the DFS itself (`findSurfaces()`). |
...
|---|---|
| `surfer` | The actual surface search tool. See [Programs](#programs) below. |
...
| File | Covers |
|---|---|
| `surfer_test.cpp` | `SurfaceFinder` end to end: exact/hand-verified surface counts on small triangulations (single tetrahedron, two tetrahedra, minimal S³, ∂Δ⁴, S⁴); unit tests for `findSurfaces(startingTriangles)` (single- and multi-triangle bases, rejecting unknown/disconnected bases), `surfaces()`, and `operator<<`; and a bounded pipeline smoke test (`CobordismBuilder::cone()` → seeded `SurfaceFinder<4>` search → `KnottedSurface::boundary()`) confirming a surface bounding a known unknotted loop is actually found. |
...
| `bogocheck.cpp` | A generic "is this the minimal triangulation?" search tool (via `Triangulation::retriangulate`), not specific to surfer. Not wired into any `CMakeLists.txt`. |
...
```
build/utils/surfer/surfer
build/utils/surfer/triangulateknot
build/utils/surfer/tests/surfer_test
build/utils/surfer/tests/cobordismbuilder_test
build/utils/surfer/tests/knotbuilder_test
build/utils/surfer/tests/triangulate_knot_table
```
...
```
surfer <PD code>
surfer { -a, --all | -b, --boundary | -c, --closed | -l, --links } <isosig>
surfer [ -v, --version | -h, --help ]
```
...
./build/utils/surfer/surfer --all baa
...
./build/utils/surfer/surfer "[[1,4,2,5],[3,6,4,1],[5,2,6,3]]"
```
...
`surfer` label, alongside the rest of Regina's test suite:

```bash
cd build
ctest -L surfer --output-on-failure
```
...
```bash
./build/utils/surfer/tests/triangulate_knot_table \
    utils/surfer/tests/pd_codes.csv [optional row limit]
```
