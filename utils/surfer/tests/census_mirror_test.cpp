// census_mirror_test.cpp
//
// Tests for mirrorCensusLookup() (see ../linkcomplement.h/.cpp): the
// SQLite-backed census mirror that resolveRecognition() checks before
// falling back to the real, mutex-guarded regina::Census::lookup(). These
// tests build a small scratch .sqlite fixture directly (rather than the
// real, multi-hundred-thousand-row mirror tools/gen_census_mirror.py
// produces), so they exercise the lookup/formatting/fallthrough logic in
// isolation, independent of whether the real mirror has been generated on
// this machine.
...
const char *FIXTURE_PATH = "census_mirror_test_fixture.sqlite";

// Builds a tiny scratch mirror at FIXTURE_PATH: one source='regina' row
...
void test_regina_hit_formats_via_rolfsen() {
    buildFixture();
    setCensusMirrorPath(FIXTURE_PATH);

    EXPECT_EQ(
        mirrorCensusLookup("fake-sig-regina-rolfsen").value_or("<MISS>"),
...
void test_regina_hit_raw_when_no_rolfsen_entry() {
    ...
    EXPECT_EQ(
        mirrorCensusLookup("fake-sig-regina-no-rolfsen").value_or("<MISS>"),
...
void test_snappy_hit_returns_verbatim() {
    ...
    EXPECT_EQ(mirrorCensusLookup("fake-sig-snappy").value_or("<MISS>"),
              std::string("K12n124"),
...
void test_miss_returns_nullopt() {
    ...
    EXPECT_EQ(mirrorCensusLookup("not-a-real-isosig").has_value(), false,
              "an isoSig absent from the mirror is a clean miss "
              "(nullopt), for callers to fall through to "
              "censusLookupName()");
}

void test_missing_file_falls_through_cleanly() {
    resetCensusMirrorForTesting();

    EXPECT_EQ(mirrorCensusLookup("fake-sig-regina-rolfsen").has_value(),
              false,
              "with no mirror file present at all, every lookup misses "
              "cleanly -- no crash, same as an isoSig genuinely absent "
              "from a present mirror");
}

void test_reenable_after_reset() {
    // Exercises the generation-bump invalidation mechanism itself:
    // MirrorConnection_'s thread-local connection, opened for the fixture
    // in an earlier test, must notice resetCensusMirrorForTesting()'s path
    ...
    EXPECT_EQ(mirrorCensusLookup("fake-sig-snappy").value_or("<MISS>"),
              std::string("K12n124"),
              "switching setCensusMirrorPath() back to a real fixture "
              "after resetCensusMirrorForTesting() re-enables lookups");
}
...
// Guards against the exact bug this mirror once had: tools/gen_census_mirror.py
...
// docstring for the full story). That mismatch made every mirror lookup
...
// full EdgeComplement::identify() path (not just mirrorCensusLookup()
...
void test_real_triangulation_isosig_key_matches_production_query() {
    ...
    EXPECT_EQ(mirrorCensusLookup(sig).value_or("<MISS>"),
              std::string("synthetic-name-for-fig8"),
              "a fixture row keyed by a REAL triangulation's isoSig() is a "
              "direct mirrorCensusLookup() hit");

    EXPECT_EQ(EdgeComplement(figureEight, {}).identify(),
              std::string("synthetic-name-for-fig8"),
              "...and the SAME key is what resolveRecognition()/"
              "EdgeComplement::identify() actually queries with, end to "
              "end (not some other signature computed independently)");
