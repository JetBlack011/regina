...
enum class BoundaryCondition : uint8_t {
    ...
    proper,    /**< Every boundary facet of the subcomplex lies on the ambient
                  boundary. */
    connected, /**< Each boundary component of the subcomplex maps to a distinct
                  ambient boundary component. */
};
...
class EmbeddedSubmanifold {
  protected:
    ...
    template <int k>
    using LowDimVec = std::vector<int>;
  ...
  private:
    ...
    /**
     * Tracking for isProper(): facetIsAmbientBoundary_ records whether each
     * ambient facet lies on the ambient triangulation's boundary.
     * badProperCount_ counts ambient facets currently at boundary-facet-count 1
     * (see faceCount_) that are not on the ambient boundary; isProper() :=
     * (badProperCount_ == 0). Updated alongside faceCount_ in
     * addFace()/removeFace(), so it needs no rollback log of its own.
     */
    ...
    std::vector<std::vector<RegistryUndoEntry>>
        registryUndoLog_; /**< Indexed by k. */

    struct Checkpoint {
        std::vector<size_t> dsuMark; /**< dsu_[k].checkpoint(), per k. */
        std::vector<size_t>
            registryMark; /**< registryUndoLog_[k].size(), per k. */
        size_t orientationDsuMark =
            0; /**< orientationDsu_.checkpoint(); see isOrientable(). */
    ...
    std::vector<Checkpoint>
        checkpoints_; /**< Indexed by ambient face index f. */

    int singularCount_ = 0;
    bool isEmbedded_ =
        true; /**< Kept in lockstep with (singularCount_ == 0). */
  ...
  protected:
    /**
     * Read-only access to isEmbedded()-tracking state
     *
    ...
    /** Returns the current submanifold-membership count of ambient facet
     * `ambientFacetIdx`. */
...
class KnottedSurface : public EmbeddedSubmanifold<4, 2> {
  public:
    /** A surface's topology, as (orientable, genus, number of punctures); see
     * surfaceTypeKey(). */
  ...
  private:
    ...
    /**
     * Per-ambient-vertex, built lazily: (pentachoron index * 5 + local
     * vertex index) -> index into that vertex's own
     * embeddings()/buildLink() tetrahedron numbering. A vertex's link never
     * changes shape across a search, so this is safe to memoize.
     */
     ...
     */
    const regina::Edge<3> *
    linkEdgeForTriangle_(const regina::Vertex<4> *ambientVertex, int f,
                         int localVertex) const;
  ...
  public:
    ...
    /** As above, then adds every face in `seedFaces`; see the base class's
     * seeded constructor. */
    KnottedSurface(const Skeleton<4, 2> &skeleton,
                   const std::vector<int> &seedFaces);
    ...
    /** As EmbeddedSubmanifold::removeFace(), also updating this class's own
     * per-vertex tracking. */
    void removeFace(int f);

    /** Returns the knot/link bounded by each boundary component of the surface,
     * keyed by boundary component index. */
     ...
     * punctures). */
    static SurfaceTypeKey
    surfaceTypeKey(const regina::Triangulation<2> &surface);
    ...
    SurfaceTypeKey surfaceType() const {
        return surfaceTypeKey(triangulation());
    }

    /**
     * The topology of the *connected* surface obtained from a possibly
     * disconnected one by discarding its closed components and joining the
     * rest with tubes. See tubedSurfaceType().
     */
    struct TubedSurfaceType {
        int genus = 0; /**< Genus of the tubed, connected surface. */
        int punctures =
            0; /**< Total boundary circles; tubing does not change this. */
        int boundedComponents =
            0; /**< Components with boundary (the ones tubed together). */
        int closedComponents =
            0; /**< Components with no boundary (discarded). */
    };

    /**
     * Classifies the connected surface obtained from `surface` by throwing
     * away every closed component and joining the remaining `c` components
     * with `c - 1` tubes.
     *
     * The genus is simply the sum. Writing `chi_i = 2 - 2g_i - b_i` for each
     * of the `c` bounded components, each tube drops the Euler characteristic
     * by 2 (remove an open disc from each of two components, glue in an
     * annulus), so the tubed surface has
     * \f[
     *   \chi = \sum_i \chi_i - 2(c-1) = 2 - 2\sum_i g_i - \sum_i b_i,
     * \f]
     * and matching that against `chi = 2 - 2G - b` with `b = sum b_i` gives
     * `G = sum g_i`.
     *
     * It's also fine to discard closed components.
     *
     * For a connected `surface` with boundary this agrees with surfaceTypeKey()
     * (one bounded component, nothing to tube).
     *
     * \pre `surface` is orientable.
     */
    static TubedSurfaceType
    tubedSurfaceType(const regina::Triangulation<2> &surface);

    /**
     * Returns this instance's shared PetalCache's stats. This is the
     * memoization hit/miss counters, plus how many addFace() calls have been
     * rejected for non-local-flatness/a transverse self-intersection; see
